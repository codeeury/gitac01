...
- position 속성
  ...
  - fixed : 고정 좌표 배치 - 원래 위치, 영역 유지를 하지 않음, 브라우저가 기준, 웹 문서와 독립
  - absolute로 설정된 박스가 부모요소를 기준으로 좌표배치가 되도록 하려면,
    부모요소에 position 속성이 설정되어 있어여 함
  - 부모요소에 position 속성이 설정되어 있지 않을 때
    - position 속성이 설정되어 있는 조상요소를 기준
    - 조상 요소 모두 position 속성이 설정되어 있지 않으면 body가 기준
